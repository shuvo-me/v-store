    ...
    timer = setTimeout(async () => {
      try {
        ...
        const response = await fetch(
          `https://dummyjson.com/products/search?q=${newTerm}&limit=10&skip=${newSkip}`,
        );
        if (response.ok) {
...
<template>
  <div class="container mx-auto px-4 py-8">
    <div class="mb-8 flex flex-col items-center justify-between lg:flex-row">
      <div class="shrink-0">
        <h1 class="mb-4 text-2xl font-bold">Products</h1>
        <p class="mb-6 text-gray-600">Explore our collection of products.</p>
      </div>
      <div class="flex flex-wrap items-center justify-end gap-4 sm:flex-nowrap">
    ...
    <div class="flex flex-col">
      ...
      <div v-else class="flex flex-grow flex-col space-y-8">
        <div
          class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5"
        >
          <product-card
        ...
        <div class="mt-auto join w-full flex-wrap justify-center">
          <button
            :class="{
              'btn join-item': true,
              'btn-active': page === skip / limit + 1,
            }"
            type="button"
            @click="skip = page * limit - limit"
